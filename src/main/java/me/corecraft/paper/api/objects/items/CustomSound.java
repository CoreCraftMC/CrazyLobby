...
public class CustomSound {
    ...
    public CustomSound(final boolean isEnabled, @NotNull final String sound, final double volume, final float pitch, @NotNull final Sound.Source source) {
        this.sound = Sound.sound(Key.key(sound), source, (float) volume, pitch);

        this.isEnabled = isEnabled || sound.isEmpty();
    }

    public CustomSound(@NotNull final CommentedConfigurationNode node, @NotNull final Sound.Source source) {
        this(node.node("toggle").getBoolean(false), node.node("type").getString("ui.button.click"),
                node.node("volume").getDouble(1.0),
